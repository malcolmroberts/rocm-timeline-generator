...
if [[ "$count_timeline" -ge "1" ]]; then 
  ...
  num_meta_lines=`cat ${tf_tl_prefix}_0.json | gawk '
    NR == 1 {
      lines = 2;
    }
    NR > 2 {
  ...
  for (( i = 0 ; i < $count_timeline; i++ )); do
    ...
    head -${num_meta_lines} ${tmp}/dict $file | \
      gawk -v num_meta=${num_meta} 'NR >= 2 && NR <= (num_meta + 1) {
        split($0, tmp, "\t");
        dict[tmp[2]] = tmp[1];
      }
      NR > (num_meta + 5) {
    ...
    while read change; do
      from_id=`echo $change | gawk '{print $2}'`;
      to_id=`echo $change | gawk '{print $NF}'`;
...
grep -a "hip-api" $log_file | \
  ...
  gawk '{print $2}' | \
  sort | uniq -c | sort -nr | \
  gawk '{if ($2 ~ /tid/ && $2 !~ /HIP/ && $1 != 2) print "hip-api toskip "$2}' \
    > $tmp_toskip
...
for tid in `/bin/grep -a "HIP initialized" $tmp_hip_api | \
  ...
  grep -a "tid:${tid}:" $tmp_hip_api | \
    ...
    gawk '{ print $NF }' | \
    sed 's/0x\(.*\))/\1/g' | \
    gawk -e '{ hex = hex2dec($0); print_meta(hex, pid, tid) }' \
...
pid_offset=`echo $(tail -1 ${timeline["hcc"]}) | \
  gawk '{if ($0 == "") print 0; else print $0}'`
